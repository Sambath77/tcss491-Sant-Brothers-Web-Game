  ...
  draw(ctx) {
    ...
    for (var i = 0; i < brickCount; i++) {
      ctx.drawImage(
        ...
        16,
        16,
        this.x + i * PARAMS.BLOCKWIDTH - this.game.camera.x,
        this.y,
        PARAMS.BLOCKWIDTH,
        PARAMS.BLOCKWIDTH
      );
...
class Brick {
  ...
  constructor(game, x, y, type, prize) {
    ...
    this.animation.push(
      new Animator(
        ASSET_MANAGER.getAsset("./sprites/bricks.png"),
        16,
        0,
        16,
        16,
    ...
    this.BB = new BoundingBox(
      this.x + PARAMS.BLOCKWIDTH / 8,
      this.y,
      (PARAMS.BLOCKWIDTH * 3) / 4,
      PARAMS.BLOCKWIDTH
    );
    this.leftBB = new BoundingBox(
      this.x,
      ...
      switch (this.prize) {
        case "Coins":
          ...
          if (Date.now() - this.startTime < 3000) {
            this.game.addEntity(
              new CoinPop(this.game, this.x, this.BB.top - PARAMS.BLOCKWIDTH)
            );
            break;
          }
        case "Coin":
        ...
        case "Growth":
          if (this.game.mario.size === 0) {
            this.game.addEntity(
              new Mushroom(this.game, this.x, this.BB.top, this, "Growth")
            );
          }
          this.type = 3;
          break;
        case "1up":
          this.game.addEntity(
            new Mushroom(this.game, this.x, this.BB.top, this, "1up")
          );
          this.type = 3;
      }
    }
  ...
  draw(ctx) {
    if (this.type) {
      this.animation[this.type].drawFrame(
        this.game.clockTick,
        ctx,
...
class Tube {
  constructor(game, x, y, size, destination) {
    ...
    this.BB = new BoundingBox(
      this.x + PARAMS.BLOCKWIDTH / 8,
      this.y,
      PARAMS.BLOCKWIDTH * 2 - (PARAMS.BLOCKWIDTH * 2) / 8,
      PARAMS.BLOCKWIDTH * (size + 1)
    );
    this.leftBB = new BoundingBox(
      this.x,
      this.y,
